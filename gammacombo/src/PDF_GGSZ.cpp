...
void PDF_GGSZ::setObservables(config c)
{
    ...
    // for HFAG average
		case babar:{
							  ...
							  setObservable("xm_dk_obs", 0.060);
							  setObservable("ym_dk_obs", 0.062);
							  setObservable("xp_dk_obs",-0.103);
							  setObservable("yp_dk_obs",-0.021);
							  break;
						  }
		case belle:{
							  obsValSource = "PRD 85 (2012) 112014";
							  setObservable("xm_dk_obs", 0.095);
							  setObservable("ym_dk_obs", 0.137);
							  setObservable("xp_dk_obs",-0.110);
							  setObservable("yp_dk_obs",-0.050);
							  break;
						  }
		case belle_md:{
		...
		case lhcb:{
							  obsValSource = "JHEP 1410 (2014) 097";
							  setObservable("xm_dk_obs", 2.5e-2);
							  setObservable("ym_dk_obs", 7.5e-2);
							  setObservable("xp_dk_obs",-7.7e-2);
							  setObservable("yp_dk_obs",-2.2e-2);
...
void PDF_GGSZ::setUncertainties(config c)
{
    ...
    case babar:{
							  ...
							  StatErr[2] = 0.037; // xp
							  StatErr[3] = 0.048; // yp
							  SystErr[0] = sqrt(sq(0.007)+sq(0.006)); // xm
							  SystErr[1] = sqrt(sq(0.004)+sq(0.006)); // ym
							  SystErr[2] = sqrt(sq(0.006)+sq(0.007)); // xp
							  SystErr[3] = sqrt(sq(0.004)+sq(0.009)); // yp
							  break;
						  }
    case belle:{
							  obsErrSource = "PRD 85 (2012) 112014";
							  StatErr[0] = 0.045; // xm
							  StatErr[1] = 0.055; // ym
							  StatErr[2] = 0.043; // xp
							  StatErr[3] = 0.053; // yp
							  SystErr[0] = sqrt(sq(0.014)+sq(0.010)); // xm
							  SystErr[1] = sqrt(sq(0.015)+sq(0.023)); // ym
							  SystErr[2] = sqrt(sq(0.014)+sq(0.007)); // xp
							  SystErr[3] = sqrt(sq(0.011)+sq(0.007)); // yp
							  break;
						  }
    case belle_md:{
							  ...
							  StatErr[2] = 0.043; // xp
							  StatErr[3] = 0.059; // yp
							  SystErr[0] = sqrt(sq(0.011)+sq(0.064)); // xm
							  SystErr[1] = sqrt(sq(0.018)+sq(0.054)); // ym
							  SystErr[2] = sqrt(sq(0.011)+sq(0.055)); // xp
							  SystErr[3] = sqrt(sq(0.018)+sq(0.063)); // yp
							  break;
						  }
		case lhcb:{
							  obsErrSource = "JHEP 1410 (2014) 097";
							  StatErr[0] = 0.025; // xm
							  StatErr[1] = 0.029; // ym
							  StatErr[2] = 0.024; // xp
							  StatErr[3] = 0.025; // yp
...
void PDF_GGSZ::setCorrelations(config c)
{
    ...
    case babar:{
                ...
                //                     xm      ym      xp      yp
							  double dataStat[]  = { 1.   , -0.020,  0.000,  0.000,		// xm
                                      -0.020,  1.   ,  0.000,  0.000,		// ym
                                       0.000,  0.000,  1.   ,  0.000,		// xp
                                       0.000,  0.000,  0.000,  1.     };// yp
							  corStatMatrix = TMatrixDSym(nObs,dataStat);
							  //                     xm      ym      xp      yp
							  double dataSyst[]  = { 1.   , -0.230, -0.020, -0.050,		// xm
                                      -0.230,  1.   ,  0.180, -0.050,		// ym
                                      -0.020,  0.180,  1.   ,  0.000,		// xp
                                      -0.050, -0.050,  0.000,  1.     };// yp
							  corSystMatrix = TMatrixDSym(nObs,dataSyst);
							  break;
						  }
    case belle:{
							  ...
							  // no syst correlation avaialble
                //                     xm      ym      xp      yp
							  double dataStat[]  = { 1.   , -0.315,  0.000,  0.000,		// xm
                                      -0.315,  1.   ,  0.000,  0.000,		// ym
                                       0.000,  0.000,  1.   ,  0.059,		// xp
                                       0.000,  0.000,  0.059,  1.     };// yp
							  corStatMatrix = TMatrixDSym(nObs,dataStat);
							  //                     xm      ym      xp      yp
