...
PDF_CrossCor_DK_hh_vs_DKst_hh::PDF_CrossCor_DK_hh_vs_DKst_hh(PDF_Abs* pdf1, PDF_Abs* pdf2, config cCor)
: PDF_CrossCorAbs(pdf1,pdf2)
{
  name = "CrossCor_DK_hh_vs_DKst_hh";
...
PDF_CrossCor_DK_hh_vs_DKst_hh::~PDF_CrossCor_DK_hh_vs_DKst_hh(){}


void PDF_CrossCor_DK_hh_vs_DKst_hh::setCorrelations(config c)
{
  resetCorrelations();
  ...
  switch(c)
  {
    ...
    default:{
      cout << "PDF_CrossCor_DK_hh_vs_DKst_hh::setCorrelations() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
      exit(1);
    }
  }
}
