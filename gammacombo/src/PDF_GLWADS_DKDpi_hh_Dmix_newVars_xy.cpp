...
PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy(config cObs, config cErr, config cCor,
...
PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::~PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy(){}


void PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::initParameters()
{
	...
	//parameters->add(*(p->get("u_dpi")));
	//parameters->add(*(p->get("v_dpi")));
	parameters->add(*(p->get("xm_dpi")));
	parameters->add(*(p->get("ym_dpi")));
	parameters->add(*(p->get("xp_dpi")));
	parameters->add(*(p->get("yp_dpi")));
...
void PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::initRelations()
{
  ...
  RooRealVar& xm       = *((RooRealVar*)p->find("xm_dpi"));
  RooRealVar& ym       = *((RooRealVar*)p->find("ym_dpi"));
  RooRealVar& xp       = *((RooRealVar*)p->find("xp_dpi"));
  RooRealVar& yp       = *((RooRealVar*)p->find("yp_dpi"));
  //RooFormulaVar *rbp  = new RooFormulaVar("rbp","rbp", "0.5 * ( TMath::Sqrt( xm_dpi*xm_dpi + ym_dpi*ym_dpi ) + TMath::Sqrt( xp_dpi*xp_dpi + yp_dpi*yp_dpi ) )", *p);
  RooFormulaVar *rbp  = new RooFormulaVar("rbp","rbp", "TMath::Sqrt( xm_dpi*xm_dpi + ym_dpi*ym_dpi )", *p);
  //RooFormulaVar *rbp  = new RooFormulaVar("rbp","rbp", "TMath::Sqrt( xp_dpi*xp_dpi + yp_dpi*yp_dpi )", *p);
  RooFormulaVar *dbp  = new RooFormulaVar("dbp","dbp", "0.5 * ( TMath::ATan( yp_dpi / xp_dpi ) + TMath::ATan( ym_dpi / xm_dpi ) )", *p);
  ...
  RooFormulaVar *g  = new RooFormulaVar("gam","gam", "0.5* ( TMath::ATan( yp_dpi / xp_dpi ) - TMath::ATan( ym_dpi / xm_dpi ) )", *p);
...
void PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::initObservables()
{
...
void PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::setObservables(config c)
{
	switch(c)
	{
		case truth:{
		...
		default:{
				 		cout << "PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::setObservables() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
						exit(1);
				 }
	}
}

void PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::setUncertainties(config c)
{
	switch(c){
		case lumi3fb:
			{
		...
		default:
			{
				cout << "PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::setUncertainties() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
				exit(1);
			}
	}
}

void PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::setCorrelations(config c)
{
	resetCorrelations();
	switch(c)
	{
		...
		default:
			{
				cout << "PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::setCorrelations() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
				exit(1);
			}
	}
}

void PDF_GLWADS_DKDpi_hh_Dmix_newVars_xy::buildPdf()
{
