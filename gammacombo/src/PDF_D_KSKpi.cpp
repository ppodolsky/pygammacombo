...
#include "PDF_D_KSKpi.h"
	...
	PDF_D_KSKpi::PDF_D_KSKpi(config cObs, config cErr, config cCor, config cType, TString d)
: PDF_Abs(3)
{
	name = "dkskpi";
	this->cType = cType;
...
PDF_D_KSKpi::~PDF_D_KSKpi(){}


void PDF_D_KSKpi::initParameters()
{
...
void PDF_D_KSKpi::initRelations()
{
...
void PDF_D_KSKpi::initObservables()
{
...
void PDF_D_KSKpi::setObservables(config c)
{
	switch(c)
	{
		case truth:{
		...
		default:{
					cout << "PDF_D_KSKpi::setObservables() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
					exit(1);
				}
	}
}
...
void PDF_D_KSKpi::setUncertainties(config c)
{
	switch(c)
	{
		case cleoFullDP:{
		...
		default:{
					cout << "PDF_D_KSKpi::setUncertainties() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
					exit(1);
				}
	}
}
...
void PDF_D_KSKpi::setCorrelations(config c)
{
	resetCorrelations();
	switch(c)
	{
		...
		default:{
					cout << "PDF_D_KSKpi::setCorrelations() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
					exit(1);
				}
	}
}
...
///
void PDF_D_KSKpi::buildPdfHistogram(TString fName, float scale, float kDobs, float dDobs)
{
	...
	obsErrSource += ", "+fName;
	corSource += ", "+fName;
	if ( !FileExists(fName) ){cout << "PDF_D_KSKpi::buildPdfHistogram() : ERROR : File not found : " << fName << endl; exit(1);}
...
void PDF_D_KSKpi::buildPdf()
{
	if ( cType==useHistogram ){
		buildPdfHistogram(this->dir+"/ExpNll/CLEO_KSKpiScan_2012_Kst_pdf.root", -3.651, 0.99, 0.43);
	}
