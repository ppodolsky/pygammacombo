...
#include "RooGLWRcpVar.h"
	...
	PDF_GLW_DK_pipipi0_2var::PDF_GLW_DK_pipipi0_2var(config cObs, config cErr, config cCor)
: PDF_Abs(2)
{
	name = "glw_like_dk_pipipi0_2var";
	initParameters();
	initRelations();
	initObservables();
	setObservables(cObs);
...
PDF_GLW_DK_pipipi0_2var::~PDF_GLW_DK_pipipi0_2var(){}


void PDF_GLW_DK_pipipi0_2var::initParameters()
{
...
void PDF_GLW_DK_pipipi0_2var::initObservables()
{
	observables = new RooArgList("observables"); ///< the order of this list must match that of the COR matrix!
	observables->add(*(new RooRealVar("aglw_dk_pipipi0_obs", "Aglw+ (DK_pipipi0) obs",  0, -1e4, 1e4)));
	observables->add(*(new RooRealVar("rglw_dk_pipipi0_obs", "Rglw+ (DK_pipipi0) obs",  1, -1e4, 1e4)));
}
...
void PDF_GLW_DK_pipipi0_2var::setUncertainties(config c)
{
	switch(c)
	{
		case lhcb:
      obsErrSource = "arXiv:1504.05442"; // http://arxiv.org/pdf/1504.05442v2.pdf
		...
		default:
			cout << "PDF_GLW_DK_pipipi0_2var::setUncertainties() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
			exit(1);
	}
}
...
void PDF_GLW_DK_pipipi0_2var::buildPdf()
{
