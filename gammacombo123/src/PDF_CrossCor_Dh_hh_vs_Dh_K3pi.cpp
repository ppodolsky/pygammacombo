...
PDF_CrossCor_Dh_hh_vs_Dh_K3pi::PDF_CrossCor_Dh_hh_vs_Dh_K3pi(PDF_Abs* pdf1, PDF_Abs* pdf2, config cCor)
: PDF_CrossCorAbs(pdf1,pdf2)
{
  name = "CrossCor_Dh_hh_vs_Dh_K3pi";
...
PDF_CrossCor_Dh_hh_vs_Dh_K3pi::~PDF_CrossCor_Dh_hh_vs_Dh_K3pi(){}


void PDF_CrossCor_Dh_hh_vs_Dh_K3pi::setCorrelations(config c)
{
  resetCorrelations();
  ...
  switch(c)
  {
    ...
    default:{
      cout << "PDF_CrossCor_Dh_hh_vs_Dh_K3pi::setCorrelations() : ERROR : config "+ConfigToTString(c)+" not found." << endl;
      exit(1);
    }
  }
}
