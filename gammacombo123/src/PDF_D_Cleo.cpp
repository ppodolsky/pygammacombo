...
void PDF_D_Cleo::initParameters()
{
	...
	parameters->add(*(p.get("rD_k2pi")));
	parameters->add(*(p.get("kD_k2pi")));
	parameters->add(*(p.get("dD_k2pi")));
...
void PDF_D_Cleo::initRelations()
{
	...
	theory->add(*(new RooFormulaVar("kD_k2pi_th", "kD_k2pi_th", "kD_k2pi", *p)));
	theory->add(*(new RooFormulaVar("dD_k2pi_th", "dD_k2pi_th", "dD_k2pi", *p)));
	...
	theory->add(*(new RooFormulaVar("B6_th", "B6_th", "B5*rD_k2pi^2*(1-(yD/rD_k2pi)*kD_k2pi*cos(dD_k2pi)+(xD/rD_k2pi)*kD_k2pi*sin(dD_k2pi)+(yD^2+xD^2)/(2*rD_k2pi^2))", *p)));
}
...
void PDF_D_Cleo::initObservables()
{
	...
	observables->add(*(new RooRealVar("kD_k2pi_obs", "kD (D#rightarrowK2#pi)",      0, -1e4, 1e4)));
	observables->add(*(new RooRealVar("dD_k2pi_obs", "#deltaD (D#rightarrowK2#pi)", 0, -1e4, 1e4, "Rad")));
...
void PDF_D_Cleo::setObservables(config c)
{
	switch(c)
	{
		...
		case cleo:
			...
			setObservable("kD_k2pi_obs",0.84);
			setObservable("dD_k2pi_obs",DegToRad(227.));
...
void PDF_D_Cleo::setUncertainties(config c)
{
	switch(c)
	{
		case cleo:
			...
			StatErr[ 2] = 0.07;          // kD_k2pi_obs
			StatErr[ 3] = DegToRad(17);  // dD_k2pi_obs
